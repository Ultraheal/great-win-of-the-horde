...
Я в данный момент использую ufo.hosting, пока что за время использования нареканий не выявил, соединение неплохое, ничего не лагает.

Можете выбрать хостинг на ваш вкус и цвет, или же зарегаться на этом, можно по моей рефералке, мне там какая-то копеечка перепадет: https://ufo.hosting/?from=914169
## Путь через Sing-box
...
Айпишники дискордовских голосовых каналов - это первые 10, остальные нужны для Telegram. Актуальный список доменов и подсетей можно подсмотреть здесь https://github.com/itdoginfo/allow-domains . Там не всегда есть все для разблокировки, но почти всегда, список со временем обновляется. Либо можно уточнить по спискам у ребят в чате itdog - ссылка в конце гайда есть
...
К сожалению решение с роутером, которое мы сделали по этому гайду не поддерживает AmneziaWG 2.0, позже я доработаю его, и допишу в гайде, но пока в текущей парадигме нужно будет поднять второй контейнер с амнезией, в итоге будет и 1.5 и 2.0. Для роутера мы будем использовать пока что 1.5(он работает исправно, с ним нет проблем на домашнем интернете), а вот для мобильных клиентов сразу перейдем на 2.0 с ним у меня все работает хорошо с мобильного интернета
...
Админка роутера будет через веб интерфейс, который выше в пункте про Амнезию, а для мобильных устройств будет с телефона через официальное приложение Амнезии. Позже когда я доработаю решение на роутере, админка останется одна с телефона, а контейнер Амнезии 1.5 можно будет удалить. В админке внутри приложения Амнезии так же можно создавать, отключать конфиги которые вы можете раздавать семье, друзьям и т.д.
