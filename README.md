# Прошивка OpenWrt
Для великой победы, нам необходима прошивка OpenWrt на роутере и доступ по ssh к нему, ниже пример с гайдом для роутера Xiaomi Router AX3200 RB01(**ПРИМЕР ТОЛЬКО ДЛЯ ЭТОГО РОУТЕРА, ЛЮБОЙ ДРУГОЙ ВЫ ТАКИМ ОБРАЗОМ МОЖЕТЕ ПРЕВРАТИТЬ В КИРПИЧ**, можете найти отдельный гайд по openWRT на свою модель, их в интернете полно), а также вот ссылка на то же самое, только в формате видео https://www.youtube.com/watch?v=jhKnUGJv81c :
